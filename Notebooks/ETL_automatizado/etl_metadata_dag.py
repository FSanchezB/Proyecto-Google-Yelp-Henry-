...
def process_and_load_data():
    ...
    google = df_final.drop(['description', 'price', 'state', 'hours', 'MISC'], axis=1)
    google = google.dropna(subset=['name', 'category'])
    google['category'] = google['category'].apply(lambda x: x[0] if isinstance(x, list) and len(x) > 0 else '')
    ...
    google['relative_results'] = google['relative_results'].apply(lambda x: x[0] if isinstance(x, list) and len(x) > 0 else '')

    # Cargamos el DataFrame a BigQuery
    table_full_id = 'xenon-mantis-431402-j8.datos_crudos.metadata'

    job_config = bigquery.LoadJobConfig( autodetect=True,
                                        source_format=bigquery.SourceFormat.CSV,
                                        write_disposition=bigquery.WriteDisposition.WRITE_TRUNCATE)
    job = bq_client.load_table_from_dataframe(google,
                                            table_full_id,
